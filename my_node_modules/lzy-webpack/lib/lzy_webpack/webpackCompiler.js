...
class Webpack {
    ...
    getNpmPackageEntry(depName) {
        ...
        // 先找到对应的模块package.json(倒序查找)
        let packageJSON;
        let depDirPath;
        let restPath = depName

        // 检查当前目录是否有packageJson,给package.json赋值
        const checkPackageJson = (restPath) => {
            if (!packageJSON) {
        ...
        while (restPath.indexOf('/') !== -1) {
            const hasPkj = checkPackageJson(restPath)
            if (hasPkj) break
        ...
        // 如果没有从子目录中找到package.json,直接从主目录找
        checkPackageJson(restPath)

        // dep可能是'react', 'react/client', 需要从package.json的export属性中找到对应的路径
        const subName = depName.replace(restPath, '')

        // 没有exports对象时直接使用main,无main情况使用index.js
        if (!packageJSON.exports) {
            return packageJSON.main
                ? path.join(depDirPath, packageJSON.main)
                : path.join(depDirPath, 'index.js')
        } else {
            // 有exports对象时主要从exports对象中查找入口
            let exportObj;

            if (subName) {// 子模块加载
                exportObj = packageJSON.exports['.' + subName]
            }
            else {// 主模块加载
                exportObj = packageJSON.exports['.']
            }
