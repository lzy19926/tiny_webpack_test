...
const { requireStatic } = require('./requireStatic')
class Webpack {
    ...
    createAssets(absolutePath) {
        ...
        traverse(ast, {
            ...
            VariableDeclarator: (path, state) => {//todo 遇到requireStatic语句  (处理path,替换AST)
                const isRequireStatic =
                    path.node.init?.type === 'CallExpression' &&
                    path.node.init?.callee?.name === 'requireStatic'

                if (isRequireStatic) {
                    const key = path.node.id.name
                    const value = path.node.init.arguments[0].value
                    const nextValue = requireStatic(value) // 删除多余public
