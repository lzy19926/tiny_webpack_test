...
class Webpack {
    ...
    getNpmPackageEntry(depName) {
        ...
        // 模块的加载循序为：_browser+mjs > module > browser+cjs > main_这个加载顺序是大部分构建工具默认的加载顺序
        let entry;
        const browserRequire = packageJSON?.exports['.']?.browser?.require
        const browserDefault = packageJSON?.exports['.']?.browser?.default
        const nodeRequire = packageJSON?.exports['.']?.default?.require
        const nodeDefault = packageJSON?.exports['.']?.default?.default
        const main = packageJSON.main

        if (browserRequire) {
            entry = path.join(depDirPath, browserRequire)
        }
        else if (browserDefault) {
            entry = path.join(depDirPath, browserDefault)
        }
        else if (nodeRequire) {
            entry = path.join(depDirPath, nodeRequire)
        }
        else if (nodeDefault) {
            entry = path.join(depDirPath, nodeDefault)
        }
        else {
            entry = path.join(depDirPath, main)
        }
    ...
    preHandleRaletivePath(relativePath, dirname) {
        // 由于需要兼容broswer端和node端,需要有broswer映射
        let absolutePath;
        if (relativePath[0] === '.') { // 以./开头的文件直接查找   添加.js后缀
            relativePath = this.addFileSuffix(relativePath)
            assetPart.absolutePath = path.join(dirname, relativePath)
        }
        else {
    ...
    createAssets(absolutePath) {
        ...
        //非js,cjs,mjs文件或者lzy不执行
        const isJSFile = /\.js$/.test(absolutePath) || /\.cjs$/.test(absolutePath) || /\.mjs$/.test(absolutePath)
