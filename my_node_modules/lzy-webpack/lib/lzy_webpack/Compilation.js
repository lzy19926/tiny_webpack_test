 ...
 * 通过Compilation对象，可以访问到打包后的模块、Chunk和Asset等信息。
 * 
...
class Compilation {
    ...
    constructor(compiler, params) {
        ...
        this.hooks = {
            ...
            CompileSync: new AsyncSeriesHook(["resolveData"]),// 手动触发下个回调,支持异步的hook
    ...
    init() {
        ...
        this.callBeforeCompileSyncHook()
    }
    ...
    createManifest(entry) {
        ...
        for (const asset of queue) {
            ...
            for (let relativePath of deps) {// 遍历文件依赖的文件
                const absolutePath = this.resolveModulePath(relativePath, dirname) // 解析路径
...
const process = {env:{NODE_ENV:'${mode}'}};
    ...
    createManifestPart(p) {
        ...
        for (const relativePath of asset.dependencies) {// 遍历文件依赖的文件
            const absolutePath = this.resolveModulePath(relativePath, dirname)
            asset.mapping[relativePath] = absolutePath //通过相对路径和绝对路径匹配 构建资源依赖图
        }
