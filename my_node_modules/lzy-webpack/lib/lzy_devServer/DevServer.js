...
class DevServer {
    ...
    initBundleCode() {
        ...
        const bundleCode = this.compilation.createBundle('serverBundle')
        const bundleCSS = this.compilation.config.plugins[1].bundleCSS(this.compilation)
    ...
    updateBundle(path) {
        ...
        if (needBundle) {
            const jsCode = this.compilation.createNewBundle(path)
            this.memoFs.writeFileSync(`/memoStatic/bundle.js`, jsCode);
            this.hash.jsHash = this.caculateHash('/memoStatic/bundle.js')
        }
        if (needBundleCss) {
            const cssCode = this.compilation.config.plugins[1].bundleCSS(this.compilation)
