import LzyReact, { renderRoute } from 'lzy-react'
...
export function Route(props, children) {
    ...
    children.forEach((child) => {
        if (child.tag === 'Route') {
            ...
            const currentPath = prevPath + child.props.path // 上层path和本层拼接
            const containerFiber = window.$$routerMap[prevPath].containerFiber // 上层Route的container
            child.props.path = currentPath       // 修改本层的path

            if (!window.$$routerMap[currentPath]) {// 注册子路由
                window.$$routerMap[currentPath] = {
                    containerFiber,
