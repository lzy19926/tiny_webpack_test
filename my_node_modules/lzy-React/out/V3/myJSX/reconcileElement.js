...
exports.renderWork_concurrent = exports.renderElement = exports.createFiberAndConnect = exports.createContainerFiber = void 0;
...
function createFiber(elementNode, parentFiber) {
    ...
    // 解析无需渲染的context等节点
    if ((0, elementType_1.isContextProviderEle)(elementNode)) {
        newFiberNode.nodeType = 'ContextProvider';
    }
    //如果tag大写 解析为FC组件节点 重新生成element
    else if ((0, elementType_1.isFunctionEle)(elementNode)) {
    ...
    //解析为text节点
    else if ((0, elementType_1.isTextEle)(elementNode)) {
        newFiberNode.nodeType = 'HostText';
        newFiberNode.text = elementNode.text;
    }
    //解析为普通dom节点
    else if ((0, elementType_1.isDomEle)(elementNode)) {
...
function createFiberAndConnect(element, parentFiber, prevSibling) {
    ...
    // 执行函数组件函数
    if ((0, elementType_1.isFunctionEle)(element)) {
        const childElement = element
            .render.call(undefined, element.props, element.children, element.fiber);
        element.children = [childElement];
    }
    // 执行forwardRef函数组件(将ref作为二号参数注入)
    if ((0, elementType_1.isForwardRefEle)(element)) {
        const childElement = element
            .render.call(undefined, element.props, element.ref, element.children, element.fiber);
        element.children = [childElement];
    }
    //-- 创建alt(顺序不能乱)
...
function mountFunctionComponent(entry) {
    ...
    const childElement = elementNode
        .render.call(undefined, elementNode.props, elementNode.children, elementNode.fiber);
