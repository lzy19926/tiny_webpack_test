...
exports.reconcileWork_sync = void 0;
...
//TODO 需要重构
// Diff核心实现 ,递归调和新老element, 复用fiber
function reconcileElement(newEle, oldEle) {
    let i = 0;
...
function reconcileWork_sync(newEle, oldEle) {
    const isSameNode = shallowCompareElement(newEle, oldEle); // 浅比较
    if (!isSameNode)
        return { isSameNode, newEle };
    switchWorkInProgressFiber(newEle, oldEle); // 将wk挂载到newEle上
    (0, renderWork_1.renderWithHook)(newEle); // 函数组件, 继续向下执行函数组件(分节点拆分)
    //! 核心diff逻辑 diff新老element树
    if (newEle.children && newEle.children.length > 0) {
        reconcileElement(newEle, oldEle);
    }
    return { isSameNode, newEle };
}
exports.reconcileWork_sync = reconcileWork_sync;
