...
exports.renderWork_concurrent = exports.renderWork_sync = exports.renderElement = exports.createContainerFiber = exports.renderWithHook = void 0;
...
// --执行带有hook的函数式组件 生成child节点
function renderWithHook(elementNode) {
    // 清空fiber上残留的钩子
    const workInProgressFiber = elementNode.fiber;
    workInProgressFiber.updateQueue = null;
    workInProgressFiber.hookIndex = 0;
    // 根据fiber状态切换钩子类型
    (0, myHook_1.switchHooksDispatcher)(workInProgressFiber.fiberFlag);
    // 执行函数组件
    if ((0, elementType_1.isFunctionEle)(elementNode)) {
        elementNode.children = [elementNode.render.call(undefined, elementNode.props, elementNode.children, elementNode.fiber)];
    }
    // 执行forwardRef函数组件(将ref作为二号参数注入)
    else if ((0, elementType_1.isForwardRefEle)(elementNode)) {
        elementNode.children = [elementNode.render.call(undefined, elementNode.props, elementNode.ref, elementNode.children, elementNode.fiber)];
    }
    return elementNode;
}
exports.renderWithHook = renderWithHook;
...
function createContainerFiber(domContainer) {
    ...
    newFiberNode.fiberFlag = 'update';
    newFiberNode.alternate.fiberFlag = 'update';
    return newFiberNode;
}
...
function renderElement(elementNode, parentFiber, prevSibling) {
    ...
    // 更改状态
    workInProgressFiber.fiberFlag = 'update';
    workInProgressFiber.alternate.fiberFlag = 'update';
    return workInProgressFiber;
}
exports.renderElement = renderElement;
// 通过Element同步render Fiber树
function renderWork_sync(rootElement, parentFiber, prevSibling) {
    if (!rootElement)
        return;
    let stack = [];
...
exports.renderWork_sync = renderWork_sync;
