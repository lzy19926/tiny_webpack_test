...
function commitPart_First(finishedWorkFiber) {
    ...
    //todo  layout阶段 执行所有layoutEffectList(如ref处理等)
    (0, concurrent_1.after_FiberNodeMutation_Hook)(() => {
        LayoutMutation(GlobalFiber_1.global.LayoutEffectList);
    });
}
...
function commitFiberNodeMutation_concurrent(EffectList, lane) {
    logger_1.logger.log('本次更新的EffectList', EffectList);
...
//! layout阶段
function LayoutMutation(layoutEffectList, lane) {
    let currentEffect = layoutEffectList.firstEffect;
    while (currentEffect !== null) {
        let effectTag = currentEffect.tag;
        let targetFiber = currentEffect.targetFiber;
        switch (effectTag) {
            case 'UpdateRef': //更新ref引用
...
function commitAttachRef(finishedWorkFiber) {
    ...
    // 先删除先前的ref
    if (!ref) {
        logger_1.logger.warn('请使用useRef创建ref容器实例');
        return;
    }
    switch (nodeType) {
        case 'AppNode' || 'FunctionComponent':
            logger_1.logger.error("无法给非Dom节点挂载ref,请使用forwardRef进行转发");
            break;
...
function callDestoryByTag(effect) {
    ...
    if (destroy !== undefined && needCallDestory) {
        destroy();
