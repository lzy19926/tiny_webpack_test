...
exports.pushLayoutEffectList = exports.pushEffectList = exports.diffUseEffect = exports.markRef = exports.scheduleFiberNode = void 0;
...
function markRef(fiber) {
    const hasRef = fiber._element.ref;
    const nodeType = fiber.nodeType;
    const canLinkRef = ['HostComponent', 'HostText', 'ForwardRef'].includes(nodeType);
    if (!canLinkRef) {
        return logger_1.logger.error(`该类型节点${nodeType}无法挂载ref,请使用forwardRef进行转发,或在Dom上挂载ref`);
    }
    if (hasRef && canLinkRef) {
        fiber.hasRef = true;
        pushLayoutEffectList('UpdateRef', fiber);
    }
}
exports.markRef = markRef;
// 添加节点逻辑
...
function scheduleUpdate(fiber) {
    ...
    const isPropsChange = diffSteyle(fiber);
    fiber.text = fiber._element.text;
    if (hasEvent || isTextChange || isPropsChange) {
...
// 计算className变更
function diffSteyle(workInProgressFiber) {
    var _a, _b;
    const curClass = (_b = (_a = workInProgressFiber._element) === null || _a === void 0 ? void 0 : _a.props) === null || _b === void 0 ? void 0 : _b.class;
    if (!curClass)
        return;
    const prevClass = workInProgressFiber.stateNode.getAttribute('class');
...
//! 创建并添加Effect到EffectList(Commit阶段遍历执行)
...
//! 创建并添加Effect到LayoutEffectList(layoutCommit阶段遍历执行)
function pushLayoutEffectList(tag, targetFiber, callback) {
    const newEffect = {
        tag,
        targetFiber,
        callback: '暂定',
    ...
    //todo 链接到全局LayoutEffectList单链表
    const LayoutEffectList = GlobalFiber_1.global.LayoutEffectList;
    if (LayoutEffectList.firstEffect === null) {
        LayoutEffectList.firstEffect = newEffect;
        LayoutEffectList.lastEffect = newEffect;
    }
    else {
        LayoutEffectList.lastEffect.next = newEffect;
        LayoutEffectList.lastEffect = newEffect;
    }
    LayoutEffectList.length += 1;
}
exports.pushLayoutEffectList = pushLayoutEffectList;
