...
function handleProps(fiber, dom) {
    ...
    for (const propName in nextProps) {
        ...
        // todo 处理其他
            ...
            !(0, utils_1.isEmpty)(nextProps[propName])
                && dom.setAttribute(propName, nextProps[propName]);
        }
    }
    // 更新当前Dom上的props
    (0, Constance_1.updateFiberPropsToDom)(dom, nextProps);
}
...
function handleEvent(propName, dom, nextProps) {
    ...
    const useSES = GlobalFiber_1.global.islzySyntheticEventSystemOpen;
    // 大小写处理
    if (propName[2].toLowerCase() === propName[2]) {
        logger_1.logger.warn(`请使用on+大写字母替代${propName},实现与原生区分`);
    }
    ...
    let prevEventHandler = (0, Constance_1.getFiberCurrentPropsFromDom)(dom)[propName];
    // 新handler,绑定为dispach,以便组成合成事件
    let eventHandler = nextProps[propName];
    if ((0, utils_1.isEmpty)(eventHandler))
        return;
    // 删除上一个监听器(更新函数引用)
    ...
    if (typeof eventHandler === 'function') {
        ...
        logger_1.logger.warn(`事件${propName}处理需要传递一个函数`);
    }
}
// todo ----------处理样式---------------
function handleStyle(propName, dom, nextProps) {
    ...
    if (propName["class"] && propName["className"]) {
        logger_1.logger.warn("请不要混用属性class和className");
    }
