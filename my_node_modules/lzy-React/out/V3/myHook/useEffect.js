...
function mountEffect(fiberFlag, hookFlags, create, deps) {
...
function updateEffect(fiberFlag, hookFlags, create, deps) {
    const fiber = GlobalFiber_1.global.workInprogressFiberNode; //! 测试
    const currentHook = (0, utils_1.getCurrentFiberHook)(fiber);
    //判断是否传入deps 不同时机执行useEffect
    const nextDeps = deps === undefined ? null : deps;
...
function shallowCompareDeps(nextDeps, prveDeps) {
    logger_1.logger.log('前后dep对比', prveDeps, nextDeps);
    // 选取最大的lenght
