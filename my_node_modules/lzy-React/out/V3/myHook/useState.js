...
exports.updateState = exports.mountState = exports.myUseState = void 0;
...
function updateUseStateHook(hook) {
    ...
    // 遍历结束 将更新后的baseState存放到hook.memorizedState上,变更hook状态
...
function myUseState(initialState) {
    ...
    let hook;
    if (fiber.fiberFlag === 'mount') {
        hook = createHook(initialState); //创建hook 添加到hook链表
    }
    else {
        // 更新情况 找到对应的hook
        hook = (0, utils_1.getCurrentFiberHook)(fiber);
    }
...
function mountState(initialState) {
    let fiber = GlobalFiber_1.global.workInprogressFiberNode;
    // mount时创建hook并更新state
    const hook = createHook(initialState);
    const baseState = updateUseStateHook(hook);
    // 返回状态和dispatch函数 (bind本次useState的fiber节点 用于从当前组件开始更新)
    return [baseState, dispatchAction.bind(null, hook.updateStateQueue, fiber)];
}
exports.mountState = mountState;
function updateState(initialState) {
    let fiber = GlobalFiber_1.global.workInprogressFiberNode;
    // update时找到hook并更新state
    let hook = (0, utils_1.getCurrentFiberHook)(fiber);
    const baseState = updateUseStateHook(hook);
    // 返回状态和dispatch函数 (bind本次useState的fiber节点 用于从当前组件开始更新)
    return [baseState, dispatchAction.bind(null, hook.updateStateQueue, fiber)];
}
