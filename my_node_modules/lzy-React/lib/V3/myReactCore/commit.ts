...
export function commitPart_First(finishedWorkFiber: FiberNode) {
  ...
  after_FiberNodeMutation_Hook(() => {
    LayoutMutation(global.LayoutEffectList)
  })
...
function commitFiberNodeMutation_concurrent(EffectList: any, lane?: any) {
  ...
  logger.log('本次更新的EffectList', EffectList);
...
//! layout阶段
function LayoutMutation(layoutEffectList: any, lane?: any) {
...
function commitAttachRef(finishedWorkFiber: FiberNode) {
  ...
  // 先删除先前的ref
  if (!ref) {
    logger.warn('请使用useRef创建ref容器实例')
    return
  }

  switch (nodeType) {
    case 'AppNode' || 'FunctionComponent':
      logger.error("无法给非Dom节点挂载ref,请使用forwardRef进行转发")
      break;
