...
import { concurrentWhile } from './concurrent'
import { insertAfter, findPrevSiblingFiber, getParentDom } from './utils'
...
function createDomElement(fiber: FiberNode) {
    ...
    if (prevDom && typeof prevDom !== 'function') {
        insertAfter(newHtmlElement, prevDom,)
    }
    // 需要判断sibling是否为dom节点
    else if (siblingDom && typeof siblingDom !== 'function') {
...
function handleProps(fiber: FiberNode, dom: HTMLElement | any) {
