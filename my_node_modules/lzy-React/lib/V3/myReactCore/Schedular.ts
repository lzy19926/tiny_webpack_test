...
import { isEmpty, findPrevSiblingFiber } from '../utils/utils'
...
export function markRef(fiber: FiberNode) {
  ...
  if (!canLinkRef) {
    return logger.error(`该类型节点${nodeType}无法挂载ref,请使用forwardRef进行转发,或在Dom上挂载ref`)
  }
...
// 计算className变更
function diffSteyle(workInProgressFiber: FiberNode) {
  const curClass = workInProgressFiber._element?.props?.class
  if (!curClass) return

  const prevClass = (workInProgressFiber.stateNode as HTMLElement).getAttribute('class')
  if (curClass !== prevClass) return true
  else return false
}
...
export function pushLayoutEffectList(tag: string, targetFiber: FiberNode, callback?: Function) {
  const newEffect = {
    tag, // Effect的类型
    targetFiber, // 需要执行的fiber节点
    callback: '暂定',
  ...
  const LayoutEffectList = global.LayoutEffectList
