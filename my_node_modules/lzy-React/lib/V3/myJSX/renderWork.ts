...
import { isFunctionEle, isForwardRefEle, getElementType, ElementType, ET } from './elementType'
import { switchHooksDispatcher } from '../myHook';
...
export function renderWithHook(elementNode: FunctionElementNode) {
  // 清空fiber上残留的钩子
  ...
  // 根据fiber状态切换钩子类型
  switchHooksDispatcher(workInProgressFiber.fiberFlag)


  // 执行函数组件
  if (isFunctionEle(elementNode)) {
    elementNode.children = [elementNode.render.call(
      undefined,
      elementNode.props,
...
function mountFunctionComponent(entry: RenderElementEntry) {
  ...
  workInProgressFiber.stateNode = (elementNode as FunctionElementNode).render
...
export function renderWork_sync(rootElement: ElementNode, parentFiber?: FiberNode, prevSibling?: ElementNode) {
