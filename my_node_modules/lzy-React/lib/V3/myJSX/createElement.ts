...
import { isElement, flatArray, isKey } from '../utils/utils'
import { isContextProvider } from '../utils/utils'
...
export function createElement(...args: any[]): any {
    ...
    if (typeof tag === 'object') {
        //todo  如果是context节点(需要优化)    
        if (isContextProvider(tag)) {
            element = {
                $$typeof: Symbol.for('lzyContextProviderElement'),
                tag: undefined,
                ref,
                key: isKey(key) ? key : 'key_' + 'lzyContextProviderElement',
                props: config,
                children,
                fiber: undefined
            }
