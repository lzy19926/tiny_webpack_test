...
import { createFiberTree, createDomElement, createTextElement, handleProps, createAlternate, createFiberTree_concurrent } from '../myJSX/createFiberTree'
...
function reconcileElementLoop(oldElement: ElementNode | TextElementNode, newElement: ElementNode | TextElementNode) {
    // 如果是文本节点  退出
    // 如果老节点是添加节点的占位 退出
    if (typeof oldElement === 'undefined') return
    if (newElement.tag === 'text' || oldElement.tag === 'text') return
    ...
    for (let i = 0; i < newElement.children.length; i++) {
        ...
        if (isPlacement) {
            ...
            const siblingBeforeFiber = newElement?.children?.[i - 1]?.fiber
...
// 生成设置key的节点Map
function createElementDiffingMap(oldElement: ElementNode) {
...
function placementFiber(newEle: ElementNode, siblingBeforeFiber: FiberNode, parentFiber: FiberNode) {
    // 创建新节点,打上tag标记
