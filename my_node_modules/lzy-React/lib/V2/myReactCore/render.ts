...
import { ElementNode } from './Interface'
...
function updateRenderPart(functionComponent: Function, workInProgressFiber: FiberNode, currentFiber: FiberNode) {
    const newElementTree = functionComponent()

    const oldElementTree = currentFiber._element

    checkPlacementAndDelete(newElementTree, oldElementTree)
}


//! 遍历检查前后两个elementTree  处理添加和删除节点
//todo 这里使用diff算法 key值快速找到添加和删除的节点
function checkPlacementAndDelete(newElementTree: ElementNode, oldElementTree: ElementNode) {
    [1, 2, 3, 4]
    [1, 2]
    newElementTree.children.forEach((newEle, index) => {
        const oldEle = oldElementTree.children[index]
        if (newEle && !)
    })
