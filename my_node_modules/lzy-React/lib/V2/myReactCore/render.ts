...
import { createBinadyElementTree, createElement } from '../myJSX/createElement'
import { updateFiberTree } from '../myJSX/updateFiberTree'
import { reconcileFiberNode, reconcileUseEffect, pushEffectList } from './Reconciler'
import { FiberNode, global } from './GlobalFiber'
import { ElementNode, TextElementNode } from './Interface'

//!--------------首屏Render方法-------------------
function firstRenderPart(elementTree: ElementNode, rootDom: HTMLElement) {
...
function updateRenderPart(functionComponent: Function, workInProgressFiber: FiberNode, currentFiber: FiberNode) {
    ...
    console.log(workInProgressFiber, '完成调和的fiber');

    // 第二轮遍历fiber树 更新打完tag的节点 
    // beginWorkFiber(workInProgressFiber)


    // console.log('本次生成的fiber', workInProgressFiber);
...
function reconcileElement(functionComponent: Function, workInProgressFiber: FiberNode) {
...
function reconcileElementLoop(oldElement: ElementNode | TextElementNode, newElement: ElementNode | TextElementNode) {
    // 生成新的vdom  从保存的map中查找是否之前有过这个vdom,
    // 如果原先没有这个vdom, 则说明需要添加节点,  同时删除map中的vdom
    // 如果所有新的vdom处理完毕  map中还剩余老的vdom 说明剩余的需要删除
    if (!newElement.children || !oldElement) return

    console.log('新ele', newElement, '老ele', oldElement);

    // 将设置key的节点推入map
    oldElement.children.forEach((child) => {
        if (typeof child.key !== 'undefined' && child.key !== null) {
            oldElement.elementMap.set(child.key, child)
        }
    })

    newElement.children.forEach((newEle, index) => {
        let hasElement: boolean
        const oldEle = oldElement?.children?.[index]

        // 判断是否存在相同节点
        hasElement = oldElement?.elementMap?.has(newEle.key)
        if (!newEle.key) {
            hasElement = true
        }
        ...
        // 如果没有oldEle,直接判断为新增
        if (!hasElement) {
            const parentFiber = oldElement.fiber
            const siblingBeforeFiber = oldElement?.children?.[index - 1]?.fiber
            // 创建新节点,打上tag标记
            const newFiberNode = createFiberTree(newEle, parentFiber, 'Placement')
            createAlternate(newFiberNode, 'Placement')
            ...
            placementFiber(newWkFiberNode, siblingBeforeFiber, parentFiber)
        }


        // 切换oldEle为newEle 并打上标记准备更新
        if (hasElement) {
            const wkFiber = oldEle.fiber
            newEle.fiber = wkFiber
            wkFiber._element = newEle
            wkFiber.alternate._element = newEle
            wkFiber.effectTag = 'Update'
            oldElement.elementMap.delete(newEle.key)
        }
    ...
    if (oldElement?.elementMap) {
        oldElement.elementMap.forEach((element) => {
...
function beginWorkFiber(workInProgressFiber: FiberNode) {
...
//! 删除该fiber节点(遍历删除)
function deleteFiber(workInProgressFiber: FiberNode, siblingBeforeFiber: FiberNode) {
    ...
    if (!siblingBeforeFiber) {
        ...
        siblingBeforeFiber.alternate._sibling = nextSibling?.alternate
    }

    pushEffectList('Delete', workInProgressFiber)
}

// 从父节点处添加节点  如果有兄弟节点,则拼接二叉树
function placementFiber(newWkFiberNode: FiberNode, siblingBeforeFiber: FiberNode, parentFiber: FiberNode) {
    ...
    if (!siblingBeforeFiber) {
        ...
        newWkFiberNode.alternate._sibling = nextSibling?.alternate
    }
}
...
//!--------------首屏Render方法-------------------
function firstRenderPart2(functionComponent: any, rootDom: HTMLElement) {
...
function updateRenderPart2(functionComponent: Function, workInProgressFiber: FiberNode, currentFiber: FiberNode) {
    ...
    const workInProgressRootFiber = updateFiberTree(newElementTree, workInProgressFiber, 'rootUpdateFiber')
...
function finishedWorkLoop(workInProgressFiber: FiberNode, rootUpdateQueue: any) {
    ...
    reconcileFiberNode(workInProgressFiber, undefined)
...
//! ----------------首屏渲染----------------------------
function render(elementTree: any, rootDom: HTMLElement) {
