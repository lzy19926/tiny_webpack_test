...
import { createFiberTree_concurrent, createDomElement, createTextElement, handleProps, createAlternate } from '../myJSX/createFiberTree'
import { createBinadyElementTree } from '../myJSX/createElement'
import { updateFiberTree_concurrent } from '../myJSX/updateFiberTree'
import { reconcileFiberNode, reconcileUseEffect } from '../myReactCore/Reconciler'
import { afterUpdateRender, concurrentWhile } from './concurrent'
import { FiberNode, global } from '../myReactCore/GlobalFiber'

//!--------------首屏Render方法-------------------
function firstRenderPart_concurrent(functionComponent: any, rootDom: HTMLElement) {

    const elementTree = createBinadyElementTree(functionComponent, undefined)

    const rootFiber = createFiberTree_concurrent(elementTree)

    return rootFiber
}

//todo 获取上一次的fiberTree 执行所有打上tag的functionComponent进行state更新 再commit   
function updateRenderPart_concurrent(functionComponent: Function, workInProgressFiber: FiberNode, currentFiber: FiberNode) {
    ...
    const workInProgressRootFiber = updateFiberTree_concurrent(newElementTree, workInProgressFiber, currentFiber, 'rootUpdateFiber')
...
//! ----------------首屏渲染----------------------------
function render_concurrent(functionComponent: any, rootDom: HTMLElement) {
    console.log('------------first render-------------');

    const beginWorkFiber = firstRenderPart_concurrent(functionComponent, rootDom)
...
//! ----------------综合updateRender方法-------------------
function updateRender_concurrent(functionComponent: Function, workInProgressFiber: FiberNode, currentFiber: FiberNode) {
    ...
    // 更新fiber树
    const beginWorkFiber = updateRenderPart_concurrent(functionComponent, workInProgressFiber, currentFiber)

    // updateRender结束后再执行commit(注册hook)
    let finishedWorkFiber;
    afterUpdateRender(() => {
        // 从下往上遍历fiber收集所有的Effects 形成环链表 上传递优先级给root
        console.log('开始finishwork');
        finishedWorkFiber = updateFinishedWork(beginWorkFiber, currentFiber)
...
export { render_concurrent, updateRender_concurrent }