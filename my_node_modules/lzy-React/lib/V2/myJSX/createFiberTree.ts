...
import { afterRender, concurrentWhile, concurrentWhileHook } from '../concurrent/concurrent'

//! 通过element树生成fiber树(普通树)
export function createFiberTree_base(elementNode: ElementNode | TextElementNode, parentFiber: FiberNode, tag?: string): FiberNode {
...
export function transformFiberTreeToBinadyTree(fiberTree: any): FiberNode {
...
//! 综合方法  创建二叉fiber树
export function createFiberTree(elementNode: ElementNode | TextElementNode, parentFiber: FiberNode, tag?: string): FiberNode {
...
//! ------------初始创建fiberTree-------------------
export function createFiberTree_concurrent(rootElement: ElementNode | TextElementNode) {
    //todo------- 遍历二叉element树----
    const stack = []
    let current: FiberNode;
    let currentAlt: FiberNode;
    let rootFiber: FiberNode;
    let rootFiberAlt: FiberNode;
    ...
    concurrentWhile(getCondition, () => {
        ...
        //执行后续操作
        if (!getCondition()) {
            concurrentWhileHook.afterRender()
        }
    })
...
//!------ 创建单个fiber节点
export function createFiber(elementNode: ElementNode | TextElementNode, parentFiber: FiberNode) {
    // 创建newFiber
    let newFiberNode = new FiberNode('mount', '$1')
...
//! -------------创建复制的alternate------------------
export function createAlternate(currentFiber: FiberNode, tag?: string): FiberNode {
...
// 创建单个alt
export function createAlternate_concurrent(currentFiber: FiberNode): FiberNode {
    let alternateFiber = new FiberNode('mount', '$2')
